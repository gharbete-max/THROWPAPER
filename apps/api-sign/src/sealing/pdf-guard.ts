 ...
 * declare hundreds of thousands of objects in a compressed stream; measured here, a 2.9 MB upload
 * held the event loop for 7.2 s and a 10.8 MB one for 25.7 s, during which this server answered
 * nobody — every organisation's signers waited on one operator's file.
 *
 ...
 * have already parsed within budget. The same few lines live in `apps/api-forms` — the products do
...
export type PdfCheck =
  ...
  /** Not a PDF pdf-lib can rewrite: damaged, encrypted, or no pages. */
...
PDFDocument.load(workerData.bytes, { updateMetadata: false, throwOnInvalidObject: true, ignoreEncryption: false })
