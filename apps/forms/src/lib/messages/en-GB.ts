/**
 * English — the reference catalogue.
 *
 * Every other locale is typed against this one, so adding a key here and forgetting it elsewhere
 * is a compile error rather than a blank label. Keep the keys in this order; the files are read
 * side by side when translating.
 *
 * A value beginning `plural:` declares plural forms — see `packages/i18n/src/catalogue.ts`.
 */

export const enGB = {
  'app.name': 'Loppa',
  'app.poweredBy': 'Powered by Loppa',
  'app.signOut': 'Sign out',
  'app.language': 'Language',
  'app.loading': 'Loading…',
  'app.loadFailed': 'This could not be loaded. Check the connection and try again.',
  'login.failed': 'The sign-in link could not be sent. Try again in a moment.',
  'login.rateLimited': 'Too many sign-in links were asked for. Wait 15 minutes, then try again.',
  'login.desktop': "On this computer you sign in from Loppa's menu: View → Sign in again.",

  'login.title': 'Sign in',
  'login.email': 'Email address',
  'login.submit': 'Send sign-in link',
  'login.sending': 'Sending…',
  'login.sent': 'If that address is registered, a sign-in link is on its way. It lasts 15 minutes.',
  'login.devHint': 'Development mode: the link is printed in the api-forms console.',

  'callback.working': 'Signing you in…',
  'callback.failed': 'That link could not be used. Request a new one.',
  'callback.retry': 'Back to sign in',

  'events.title': 'Events',
  'events.new': 'New event',
  'events.empty': 'No events yet.',
  'events.untranslated': 'Missing translation: {locales}',
  'events.registrationOpen': 'Registration open',
  'events.registrationClosed': 'Registration closed',
  'events.capacity': 'plural:one {count} place | other {count} places',
  'events.registered': 'plural:one {count} registered | other {count} registered',
  'events.registeredOf': '{count} of {capacity} registered',
  'events.archive': 'Archive',
  'events.archiveConfirm': 'Archive this event? It can be restored.',
  'events.adminOnly': 'Only administrators can change events.',

  'event.name': 'Name',
  'event.description': 'Description',
  'event.startsAt': 'Starts',
  'event.endsAt': 'Ends',
  'event.venueName': 'Venue',
  'event.venueAddress': 'Address',
  'event.capacity': 'Capacity',
  'event.registrationClosesAt': 'Registration closes',
  'event.status': 'Status',
  'event.status.draft': 'Draft',
  'event.status.open': 'Open',
  'event.status.closed': 'Closed',
  'event.status.archived': 'Archived',
  'event.save': 'Save',
  'event.saving': 'Saving…',
  'event.cancel': 'Cancel',
  'event.createTitle': 'New event',
  'event.editTitle': 'Edit event',
  'event.notFound': 'There is no event with that address.',

  'nav.events': 'Events',
  'nav.forms': 'Forms',
  'nav.inbox': 'Responses',
  'nav.users': 'Users',

  'nav.invoices': 'Invoices',
  'invoices.heading': 'Invoices',
  'invoices.outstanding': 'Outstanding',
  'invoices.number': 'Invoice',
  'invoices.recipient': 'Recipient',
  'invoices.subject': 'For',
  'invoices.due': 'Due',
  'invoices.total': 'Total',
  'invoices.overdue': 'Overdue',
  'invoices.open': 'Open',
  'invoices.pdf': 'PDF',
  'invoices.empty': 'No invoices yet',
  'invoices.emptyHint': 'Invoices appear here once a run has been raised.',
  'invoiceStatus.draft': 'Draft',
  'invoiceStatus.issued': 'Issued',
  'invoiceStatus.sent': 'Sent',
  'invoiceStatus.paid': 'Paid',
  'invoiceStatus.cancelled': 'Cancelled',

  'scope.active': 'All',
  'scope.mine': 'My forms',
  'scope.shared': 'Shared with me',
  'scope.trash': 'Bin',
  'scope.all': 'Whole organisation',
  'scope.emptyMine': 'You have not made any forms yet.',
  'scope.emptyShared': 'Nobody has shared a form with you.',
  'scope.emptyTrash': 'The bin is empty.',
  'scope.theirs.mine': 'Their forms',
  'scope.theirs.shared': 'Shared with them',
  'scope.theirs.trash': 'Their bin',
  'scope.theirs.active': 'All',
  'scope.theirs.all': 'Whole organisation',

  'forms.owner': 'By {name}',
  'forms.ownerYou': 'By you',
  'forms.ownerNobody': 'Whole organisation',
  'forms.access.viewer': 'Can view',
  'forms.access.editor': 'Can edit',
  'forms.access.admin': 'Administrator',
  'forms.trash': 'Move to bin',
  'forms.restore': 'Restore',
  'forms.deleteForever': 'Delete for ever',
  'forms.trashedAt': 'In the bin since {date}',
  'forms.confirmTrash':
    'Move “{title}” to the bin? It stops accepting responses, and you can restore it later.',
  'forms.confirmDelete':
    'plural:one Permanently delete “{title}” and its {count} response? This cannot be undone. | other Permanently delete “{title}” and all {count} responses? This cannot be undone.',

  'share.title': 'Share',
  'share.with': 'plural:one Shared with {count} person | other Shared with {count} people',
  'share.email': 'Email address',
  'share.emailHint': 'Must be somebody already in the organisation.',
  'share.role': 'Permission',
  'share.add': 'Share',
  'share.remove': 'Remove',
  'share.none': 'Not shared with anybody.',

  'inbox.title': 'Responses',
  'inbox.intro': 'The latest responses across every form you can see.',
  'inbox.empty': 'No responses yet.',
  'inbox.loadFailed': 'Responses could not be loaded.',
  'inbox.partial': 'In progress',
  'inbox.search': 'Search',
  'inbox.count': 'plural:one {count} response | other {count} responses',

  'users.title': 'Users',
  'users.intro': 'Everybody in the organisation. Open one to see their forms and bin.',
  'users.forms': 'plural:one {count} form | other {count} forms',
  'users.inBin': '{count} in the bin',
  'users.disabled': 'Disabled',
  'users.role.admin': 'Administrator',
  'users.role.operator': 'Member',
  'users.open': 'View workspace',
  'users.back': 'All users',
  'users.viewingAs':
    "You are viewing {name}'s workspace as yourself. Anything you do is logged as you.",
  'users.notFound': 'No such user.',
  'users.add': 'Add somebody',
  'users.addName': 'Name',
  'users.addEmail': 'Email address',
  'users.addRole': 'Role',
  'users.addHint': 'They receive a sign-in link at this address. There is nothing to accept.',
  'users.addSubmit': 'Add',
  'users.addCancel': 'Cancel',
  'users.disable': 'Disable',
  'users.enable': 'Enable',
  'users.makeAdmin': 'Make administrator',
  'users.makeMember': 'Make member',
  'users.confirmDisable':
    'Disable {name}? They keep their forms and their place in the log, and cannot sign in until you enable them again.',
  'users.confirmRole': 'Change what {name} may do?',
  'users.errorEmailTaken': 'Somebody with that address is already here.',
  'users.errorLastAdmin': 'Promote another administrator first — an organisation must keep one.',
  'users.errorFailed': 'That did not work. Try again.',

  'forms.title': 'Forms',
  'forms.new': 'New form',
  'forms.untitled': 'Untitled form',
  'forms.empty': 'No forms yet.',
  'forms.slug': 'Link address',
  'forms.slugHint': 'Becomes /f/<address>. Lower case, digits and hyphens.',
  'forms.formTitle': 'Title',
  'forms.create': 'Create',
  'forms.status.draft': 'Draft',
  'forms.status.published': 'Published',
  'forms.status.closed': 'Closed',
  'forms.status.archived': 'Archived',
  'forms.version': 'Version {n}',
  'forms.unpublished': 'Never published',
  'forms.responses': 'plural:one {count} response | other {count} responses',
  'forms.notFound': 'That form does not exist, or you do not have access to it.',
  'forms.viewResponses': 'View responses',
  'forms.copyLink': 'Copy link',
  'forms.copied': 'Copied',
  'forms.copyFailed': 'Could not copy',
  'forms.untranslated': 'Missing translation: {locales}',

  'builder.palette': 'Fields',
  'builder.properties': 'Properties',
  'builder.empty': 'Add a field from the panel on the left.',
  'builder.selectField': 'Select a field to edit it.',
  'builder.saved': 'Saved',
  'builder.saving': 'Saving…',
  'builder.unsaved': 'Unsaved',
  'builder.publish': 'Publish',
  'builder.publishing': 'Publishing…',
  'builder.history': 'Versions',
  'builder.restore': 'Restore',
  'builder.restoreConfirm': 'Replace the draft with version {n}? Anything unpublished is lost.',
  'builder.undo': 'Undo (Ctrl+Z)',
  'builder.redo': 'Redo (Ctrl+Shift+Z)',
  'builder.remove': 'Remove',
  'builder.translationsComplete': 'All languages complete',
  'builder.translationsMissing': '{locale}: {n} missing',
  'builder.overrideBody':
    'Translations are missing for {locales}. Publish anyway? Visitors see the fallback.',

  'field.key': 'Key',
  'field.keyHint': 'Used in the export. Do not change after publishing.',
  'field.label': 'Label',
  'field.helpText': 'Help text',
  'field.placeholder': 'Placeholder',
  'field.placeholderHint': 'Grey example text inside the box. Not a replacement for help text.',
  'field.required': 'Required',
  'field.width': 'Width',
  'field.widthHint': 'Every field takes the full width on a phone.',
  'field.width.full': 'Full row',
  'field.width.half': 'Half row',
  'field.width.third': 'One third',
  'field.options': 'Options',
  'field.appearance': 'Appearance',
  'field.appearanceHint':
    'Changes how the question looks. Answers are stored the same way either way.',
  'field.appearance.dropdown': 'Dropdown',
  'field.appearance.star': 'Stars',
  'field.appearance.number': 'Numbers',
  'field.scale': 'Scale',
  'field.scaleHint': 'How many points, 2 to 10. Five stars, or ten numbers.',
  'field.minLabel': 'Label at the low end',
  'field.maxLabel': 'Label at the high end',
  'field.appearance.radio': 'Radio buttons',
  'field.appearance.checkboxes': 'Checkboxes',
  'field.appearance.buttons': 'Buttons',
  'field.appearance.cards': 'Cards',
  'field.choiceStyle': 'Choice style',
  'field.choiceStyleHint':
    'Whatever you pick, every option stays large enough to tap and the chosen one is always clearly marked.',
  'field.shape': 'Shape',
  'field.shape.theme': 'As in Brand',
  'field.shape.square': 'Square',
  'field.shape.rounded': 'Rounded',
  'field.shape.pill': 'Pill',
  'field.size': 'Size',
  'field.size.regular': 'Regular',
  'field.size.large': 'Large',
  'field.accent': 'Colour of the chosen answer',
  'field.columns': 'Columns',
  'field.columns.auto': 'Automatic',
  'field.addOption': 'Add option',
  'field.optionValue': 'Value',
  'field.content': 'Content',
  'field.fromParameter': 'From URL parameter',

  'fieldType.short_text': 'Short text',
  'fieldType.long_text': 'Long text',
  'fieldType.number': 'Number',
  'fieldType.email': 'Email',
  'fieldType.phone': 'Phone',
  'fieldType.date': 'Date',
  'fieldType.single_select': 'Single select',
  'fieldType.multi_select': 'Multi select',
  'fieldType.yes_no': 'Yes/No',
  'fieldType.rating': 'Rating',
  'fieldType.time': 'Time',
  'fieldType.file': 'File upload',
  'fieldType.signature': 'Signature',
  'fieldType.section_break': 'Section',
  'fieldType.page_break': 'Page break',
  'fieldType.rich_text': 'Text block',
  'fieldType.image': 'Image',
  'fieldType.link': 'Link',

  'field.href': 'Link address',
  'field.hrefHint': 'Opens in a new tab, so answers already filled in are not lost.',
  'url.placeholder': 'https://',
  'field.linkAppearance': 'Shown as',
  'field.linkAppearance.button': 'Button',
  'field.linkAppearance.link': 'Text link',

  'fieldType.shape': 'Shape',
  'fieldType.drawing': 'Drawing',
  'palette.decoration': 'Decoration',
  'field.shapeKind': 'Shape',
  'field.shapeKind.rectangle': 'Rectangle',
  'field.shapeKind.ellipse': 'Ellipse',
  'field.shapeKind.line': 'Line',
  'field.shapeKind.arrow': 'Arrow',
  'field.shapeKind.divider': 'Divider',
  'field.fill': 'Fill',
  'field.stroke': 'Line colour',
  'field.strokeWidth': 'Line thickness',
  'field.radius': 'Corner radius',
  'field.shapeHeight': 'Height',
  'field.dashed': 'Dashed',
  'field.colour.none': 'None',
  'field.colour.custom': 'Custom…',
  'field.colourHint':
    'Brand colours follow the organisation’s palette, so restyling the brand restyles this too.',
  'draw.title': 'Draw',
  'draw.hint': 'Draw with a finger, a pen or the mouse.',
  'draw.clear': 'Clear',
  'draw.undoStroke': 'Undo last stroke',
  'draw.empty': 'Nothing drawn yet.',
  'draw.strokes': 'plural:one {count} stroke | other {count} strokes',
  'decoration.note': 'Decoration only. It collects no answer and never appears in the export.',

  'field.drawArea': 'Drawing area',
  'field.drawArea.wide': 'Wide',
  'field.drawArea.square': 'Square',
  'field.drawArea.tall': 'Tall',
  'field.drawAreaLocked': 'Clear the drawing to change its shape.',

  'appearance.label': 'Appearance',
  'appearance.system': 'System',
  'appearance.light': 'Light',
  'appearance.dark': 'Dark',

  'palette.title': 'Go to',
  'palette.placeholder': 'Search screens…',
  'palette.empty': 'Nothing matches.',

  'fieldType.hidden': 'Hidden field',

  'submissions.title': 'Responses ({n})',
  'submissions.empty': 'No responses yet.',
  'submissions.notPublished': 'This form is not published yet, so there are no responses to show.',
  'submissions.stat.complete': 'Complete',
  'submissions.stat.partial': 'Started',
  'submissions.stat.languages': 'Languages',
  'submissions.stat.latest': 'Latest response',
  'submissions.search': 'Search',
  'submissions.columns': 'Columns',
  'submissions.separator': 'Separator',
  'submissions.exportCsv': 'Export CSV',
  'submissions.exportXlsx': 'Export Excel',
  'submissions.column.reference': 'Reference',
  'submissions.column.submittedAt': 'Submitted',
  'submissions.column.locale': 'Language',
  'submissions.column.status': 'Status',

  'nav.checkin': 'Check-in',
  'nav.brand': 'Brand',

  'problem.duplicate-key': 'The field key "{key}" is used more than once.',
  'problem.no-answerable-fields': 'The form does not collect any answers yet.',
  'problem.empty-options': 'A choice question has no options.',
  'problem.condition-unknown-field': 'A condition asks about "{key}", which no field is called.',
  'problem.condition-forward-reference':
    'A condition asks about "{key}", which comes further down the form.',
  'problem.unsafe-pattern':
    'A format rule can take an unreasonable time to check. Avoid a repeat inside a repeated group, such as (a+)+.',

  'settings.heading': 'Form settings',
  'settings.submitLabel': 'Submit button wording',
  'settings.submitLabelHint': 'Leave blank for the standard wording.',
  'settings.confirmationMessage': 'Thank-you message',
  'settings.confirmationMessageHint':
    'Shown after the form is sent, alongside the reference number.',
  'settings.redirectUrl': 'Send them on to',
  'settings.redirectUrlHint':
    'Optional. Replaces the thank-you screen. The reference is carried in the link.',
  'settings.locales': 'Languages this form is offered in',
  'settings.localesHint':
    'Tick more than one and the form gets its own language switcher. Leave all unticked for every language the organisation publishes in. You write each version yourself, on the translation tab.',
  'settings.showProgress': 'Show how far through they are',
  'settings.allowSaveAndResume': 'Allow saving and continuing later',
  'settings.duplicateControl': 'Duplicates',
  'settings.duplicateControlHint': 'Whether the same email address may answer more than once.',
  'settings.duplicateControl.email': 'One response per email address',
  'settings.duplicateControl.none': 'Any number of responses',
  'settings.identity': 'Offer to confirm with e-ID after sending (optional for the person)',
  'settings.identityHint':
    'Their answers are saved first; confirming is a step they may take or skip. Where no e-ID provider is connected, the form says so and is finished without it.',
  'settings.identityPaper':
    'Not on a form made from paper: its document is the paper itself, which has nowhere to show a confirmation.',

  'public.progress': 'Step {n} of {total}',

  'brand.themes': 'Themes',
  'brand.themesHint':
    'Pick a theme to start from, then adjust it. Every one passes the contrast check.',
  'brand.themeSampleLabel': 'Question',
  'brand.themeSampleButton': 'Send',

  'theme.default': 'Default',
  'theme.midnight': 'Midnight',
  'theme.minimal': 'Minimal',
  'theme.garden': 'Garden',
  'theme.bold': 'Bold',

  'file.hint': '{kinds}, up to {size} MB.',
  'file.accept.image': 'Images',
  'file.accept.pdf': 'PDF files',
  'file.accept.both': 'Images or PDF',
  'file.uploading': 'Uploading {name}…',
  'file.remove': 'Remove',
  'file.error.too-large': 'That file is too large.',
  'file.error.empty': 'That file is empty.',
  'file.error.svg-not-supported': 'SVG cannot be attached. Send a PNG or a JPEG.',
  'file.error.unsupported-format': 'That kind of file cannot be attached.',
  'file.error.not-accepted-here': 'This question does not take that kind of file.',
  'file.error.no-such-field': 'This question does not take files.',
  'file.error.closed': 'This form is no longer accepting answers.',
  'file.error.no-file': 'No file was chosen.',
  'file.error.network': 'The upload failed. Try again.',

  'field.statement': 'What is being signed',
  'field.statementHint':
    'Shown above the signing area. For example: "I confirm the above is correct."',

  'signature.clear': 'Clear',
  'signature.apply': 'Use this signature',
  'signature.saving': 'Saving…',
  'signature.saved': 'Signed',
  'signature.failed': 'The signature could not be saved. Try again.',
  'signature.typeInstead': 'Or type your name',

  'field.accept': 'Accepted file types',
  'field.maxBytes': 'Largest size (MB)',

  'validation.file': 'Attach a file.',

  'visibility.heading': 'Show only when…',
  'visibility.count': '{n} conditions',
  'visibility.intro':
    'The field appears only when the conditions hold. Otherwise it is skipped entirely.',
  'visibility.needsEarlierField':
    'Put a question above this one first — a condition can only read an earlier answer.',
  'visibility.match': 'Require',
  'visibility.match.all': 'All conditions',
  'visibility.match.any': 'Any condition',
  'visibility.field': 'Question',
  'visibility.operator': 'Comparison',
  'visibility.value': 'Value',
  'visibility.add': 'Add condition',
  'visibility.remove': 'Remove condition',
  'visibility.missingField': '{key} — no longer above this field',
  'visibility.operator.equals': 'is',
  'visibility.operator.notEquals': 'is not',
  'visibility.operator.contains': 'contains',
  'visibility.operator.answered': 'is answered',
  'visibility.operator.empty': 'is empty',
  'visibility.operator.greaterThan': 'is greater than',
  'visibility.operator.lessThan': 'is less than',

  'rules.heading': 'Answer rules',
  'rules.intro': 'Leave blank for no rule.',
  'rules.minLength': 'Minimum characters',
  'rules.maxLength': 'Maximum characters',
  'rules.rows': 'Height in rows',
  'rules.min': 'Lowest value',
  'rules.max': 'Highest value',
  'rules.minDate': 'Earliest date',
  'rules.maxDate': 'Latest date',
  'rules.minTime': 'Earliest time',
  'rules.maxTime': 'Latest time',
  'rules.decimals': 'Decimal places',
  'rules.minSelected': 'Minimum choices',
  'rules.maxSelected': 'Maximum choices',
  'rules.defaultValue': 'Default value',
  'rules.defaultValueHint': 'Used when the link carries no parameter.',
  'rules.pattern': 'Format',
  'rules.pattern.none': 'Anything',
  'rules.pattern.letters': 'Letters only',
  'rules.pattern.digits': 'Digits only',
  'rules.pattern.alphanumeric': 'Letters and digits',
  'rules.pattern.postcodeSe': 'Swedish postcode',
  'rules.pattern.url': 'Web address',
  'rules.pattern.custom': 'Custom pattern',
  'rules.pattern.expression': 'Regular expression',
  'rules.pattern.expressionHint': 'Matched against the whole answer. Do not add ^ or $.',

  'nav.sections': 'Sections',

  'field.optionLabel': 'Option text',
  'field.addLanguage': '{locale}',
  'field.optionValueHint': 'Stored in the export. Do not change it once answers exist.',
  'field.removeOption': 'Remove',
  'field.defaultLabel': 'New question',
  'field.defaultSection': 'New section',
  'field.defaultText': 'Write your text here.',
  'field.defaultOption': 'Option {n}',

  'confirm.title': 'Confirm',
  'confirm.cancel': 'Cancel',
  'confirm.confirm': 'Yes, continue',

  'builder.duplicate': 'Duplicate',
  'builder.removeYes': 'Remove',

  'forms.edit': 'Edit form',

  'palette.text': 'Text',
  'palette.numbers': 'Numbers and dates',
  'palette.attachments': 'Files',
  'palette.choice': 'Choices',
  'palette.layout': 'Layout',

  'builder.addHint': 'A new field lands directly after the one you have selected.',
  'builder.needsLabel': 'Needs a question',
  'builder.moveUp': 'Move up',
  'builder.moveDown': 'Move down',
  'builder.resize': 'Resize',
  'builder.viewPreview': 'Whole form',

  'preview.empty': 'Add a field and the form appears here.',
  'preview.page': 'Page {current} of {total}',
  'preview.note':
    'This is what the form looks like to the person filling it in. Nothing here is saved.',

  'field.labelPlaceholder': 'For example: What is your name?',
  'field.formatHint': 'Select some text, then press B, I or U.',
  'field.advanced': 'Advanced',

  'templates.heading': 'Start from',
  'templates.intro':
    'Templates are starting points — change anything that does not fit. Legal, medical and tax forms are deliberately absent; that wording has to come from a person.',
  'templates.blank': 'Blank form',
  'templates.blankHint': 'Start from nothing.',
  'templates.fields': 'fields',

  'brand.title': 'Brand',
  'brand.intro':
    'Colours and shape for everything the organisation sends out. Forms, PDFs and email all read the same set.',
  'brand.readOnly': 'Only admins can change the brand.',
  'brand.loadFailed': 'The brand could not be loaded.',
  'brand.save': 'Save',
  'brand.saving': 'Saving…',
  'brand.reset': 'Reset to default',
  'brand.clientMode': 'Client mode',
  'brand.clientModeOn': 'Use our own logo and name',
  'brand.clientModeHint':
    'Replaces the Loppa mark in the app and on the sign-in screen. Your published forms already use your brand.',
  'brand.wordmark': 'Name in the corner',
  'brand.colours': 'Colours',
  'brand.logo': 'Logo',

  'image.add': 'Add an image',
  'image.replace': 'Replace the image',
  'image.remove': 'Remove the image',
  'image.uploading': 'Uploading…',
  'image.previewAlt': 'The chosen image',
  'image.hint': 'PNG, JPEG, WebP or GIF, up to 2MB. SVG is not supported.',
  'image.alt': 'Alt text',
  'image.altHint':
    'Describe the image for somebody who cannot see it. Leave it empty if it is decorative.',
  'image.maxWidth': 'Maximum width',
  'image.optionImage': 'Image for this option',

  'brand.logoAdd': 'Add a logo',
  'brand.logoReplace': 'Replace the logo',
  'brand.logoRemove': 'Remove the logo',
  'brand.logoAlt': 'The organisation logo',
  'brand.logoHint': 'PNG, JPEG, WebP or GIF, up to 2MB. SVG is not supported.',
  'brand.shape': 'Shape and size',
  'brand.type': 'Typeface',
  'brand.labels': 'Question text',
  'brand.borderWidth': 'Border thickness',
  'brand.spacingUnit': 'Spacing',
  'brand.spacingHint': 'Everything else is measured in this unit.',
  'brand.controlHeight': 'Button and field height',
  'brand.controlHeightHint':
    'Touch screens always use at least 44px, or controls become hard to hit.',
  'brand.contentWidth': 'Form width',
  'brand.contentWidthHint': 'How wide the form may get on a large screen.',
  'brand.bodyFont': 'Font',
  'brand.lineHeight': 'Line height',
  'brand.scaleRatio': 'Heading scale',
  'brand.scaleRatioHint': 'How much larger each heading level gets.',
  'brand.bold': 'Bold',
  'brand.italic': 'Italic',
  'brand.underline': 'Underline',
  'brand.logoColour': 'Found this colour in the logo',
  'brand.logoColourApply': 'Use as primary',
  'brand.radius': 'Corner radius',
  'brand.baseSize': 'Text size',
  'brand.preview': 'Preview',
  'brand.contrast': 'Contrast',
  'brand.contrastOk': 'Every combination is readable.',
  'brand.contrastNeeds': 'needs',
  'brand.contrastAdvisory': 'Warnings never block a save. They only say what will be hard to read.',
  'brand.colour.primary': 'Primary',
  'brand.colour.secondary': 'Secondary',
  'brand.colour.accent': 'Accent',
  'brand.colour.background': 'Background',
  'brand.colour.surface': 'Surface',
  'brand.colour.text': 'Text',
  'brand.colour.muted': 'Muted text',
  'brand.colour.border': 'Border',
  'brand.colour.success': 'Success',
  'brand.colour.warning': 'Warning',
  'brand.colour.danger': 'Error',
  'brand.hint.primary': 'Buttons and selected choices.',
  'brand.hint.secondary': 'Secondary buttons.',
  'brand.hint.accent': 'Decorative. Do not use it for text.',
  'brand.hint.background': 'The page behind everything.',
  'brand.hint.surface': 'Cards and panels.',
  'brand.hint.text': 'Body text.',
  'brand.hint.muted': 'Help text and labels.',
  'brand.hint.border': 'Field edges.',
  'brand.hint.success': 'Confirmations.',
  'brand.hint.warning': 'Warnings.',
  'brand.hint.danger': 'Error messages.',
  'brand.previewHeading': 'Registration',
  'brand.previewBody': 'This is what a form looks like to the person filling it in.',
  'brand.previewField': 'Name',
  'brand.previewChoice': 'Meal',
  'brand.previewOptionA': 'Standard',
  'brand.previewOptionB': 'Vegetarian',
  'brand.previewSubmit': 'Register me',
  'brand.previewSecondary': 'Save draft',
  'brand.previewError': 'This is what an error looks like.',

  'demo.banner': 'Demo mode — nothing is saved and no email is ever sent.',
  'demo.reset': 'Reset demo data',
  'demo.signInAs': 'Sign in as {role}',
  'demo.signInFailed': 'Could not sign in. Is the API running?',
  'demo.title': 'Open the demo',
  'demo.signInHint': 'Pick who to be. Nothing here is real, and nothing is saved.',

  'checkin.title': 'Check-in',
  'checkin.counts': '{checkedIn} of {registered} checked in',
  'checkin.reference': 'Reference',
  'checkin.referencePlaceholder': 'AB12-CD34',
  'checkin.check': 'Check in',
  'checkin.checking': 'Checking…',
  'checkin.startCamera': 'Start camera',
  'checkin.stopCamera': 'Stop camera',
  'checkin.cameraUnavailable': 'The camera is unavailable — type the reference instead.',
  'checkin.arrivedAt': 'Arrived {time}',
  'checkin.leave': 'Leave the door',
  'checkin.ofRegistered': 'of {registered}',
  'checkin.idle': 'Scan a card, or type the reference',
  'checkin.recent': 'Last arrivals',
  'checkin.undo': 'Undo',
  'checkin.undoConfirm': 'Undo the check-in for {name}?',
  'checkin.offline': 'Offline — scans cannot be checked until the connection is back.',
  'checkin.outcome.admitted': 'Welcome',
  'checkin.outcome.already': 'Already checked in',
  'checkin.outcome.revoked': 'Registration withdrawn',
  'checkin.outcome.wrong-event': 'Wrong event',
  'checkin.outcome.failed': 'Could not be checked',
  'checkin.outcome.not-found': 'Not found',
  'checkin.outcome.bad-signature': 'Invalid card',
  'checkin.outcome.undone': 'Check-in undone',

  'attendance.title': 'Attendance',
  'attendance.loadFailed': 'The attendance list could not be loaded.',
  'attendance.retry': 'Try again',
  'attendance.registered': 'Registered',
  'attendance.checkedIn': 'Checked in',
  'attendance.noShow': 'No-shows',
  'attendance.revoked': 'Withdrawn',
  'attendance.revokeConfirm': 'Withdraw the registration for {name}? The place is freed up again.',
  'attendance.all': 'All',
  'attendance.onlyNoShow': 'No-shows only',
  'attendance.exportCsv': 'Export CSV',
  'attendance.openCheckIn': 'Open check-in',
  'attendance.column.reference': 'Reference',
  'attendance.column.name': 'Name',
  'attendance.column.email': 'Email',
  'attendance.column.checkedInAt': 'Checked in',
  'attendance.column.status': 'Status',
  'attendance.status.arrived': 'Arrived',
  'attendance.status.expected': 'Expected',
  'attendance.status.revoked': 'Withdrawn',

  'public.next': 'Next',
  'wizard.step': 'Question {n}',
  'wizard.done': 'Done',
  'wizard.review': 'Here is what that gives you',
  'wizard.open': 'Open it in the editor',
  'wizard.next': 'Next',
  'wizard.back': 'Back',
  'wizard.advanced': 'Build it myself',
  'public.back': 'Back',
  'public.complete': 'Complete',
  'public.submitting': 'Submitting…',
  'public.save': 'Save and continue later',
  'public.saving': 'Saving…',
  'public.savedTitle': 'Saved',
  'public.savedBody': 'Keep the link below. It lasts 30 days.',
  'public.copy': 'Copy link',
  'public.copied': 'Copied',
  'public.thanks': 'Thank you.',
  'public.reference': 'Your reference: {reference}',
  'public.confirmation': 'A confirmation is on its way to {email}.',
  'public.confirmationWithCard':
    'A confirmation with your admission card is on its way to {email}.',
  'public.closed.not-open-yet': 'This form has not opened yet.',
  'public.closed.full': 'All places are taken.',
  'public.closed.unpublished': 'Form not found.',
  'public.rejected.duplicate': 'That address has already been registered.',
  'public.rejected.full': 'The places ran out while you were filling this in.',
  'public.rejected.closed': 'The form closed while you were filling this in.',
  'public.rejected.offline':
    'Your answers are still here, but they could not be sent. Check your connection and try again.',
  'public.rejected.error':
    'Something went wrong at our end. Your answers are still here — please try again.',
  'public.notFound': 'Form not found.',
  'public.retry': 'Try again',
  'public.loadFailed': 'The form could not be loaded.',
  'public.notFoundHint':
    'Check the link you were given, or ask the organiser for a new one. It may have been withdrawn.',
  'public.yes': 'Yes',
  'public.no': 'No',
  'public.choose': 'Choose…',
  'public.finished.sent': 'Your answers have been sent to {organisation}.',
  'public.document.preparing': 'Preparing your document…',
  'public.document.ready': 'Your document is ready',
  'public.document.unavailable': "Your document isn't available right now",
  'public.document.download': 'Download PDF',
  'public.document.open': 'Open',
  'public.document.email': 'Email it',
  'public.document.failed': 'The PDF could not be made just now. Your answers are saved.',
  'public.document.expired':
    'This copy is no longer available here. Your answers are saved under reference {reference}.',
  'public.document.retry': 'Try again',
  'public.email.title': 'Email the document',
  'public.email.draft': 'Open a draft in {program}',
  'public.email.drafting': 'Opening {program}…',
  'public.email.draftOpened':
    'The draft is open in {program} with the PDF attached. Add who it is for and send it from there.',
  'public.email.draftFailed':
    '{program} could not open a draft. Use one of the ways below instead.',
  'public.email.share': 'Share or email…',
  'public.email.app': 'Open my email app',
  'public.email.attachNote':
    'Your email app opens with the message written. Attach {filename} yourself before sending — a web page cannot attach files to an email.',
  'public.email.copy': 'Copy message text',
  'public.email.copied': 'Copied',
  'public.email.subject': '{title} — {reference}',
  'public.email.body': 'Document: {filename}\nReference: {reference}',
  'public.again': 'Fill in again',
  'public.eid.title': 'Confirm who you are (optional)',
  'public.eid.unavailable':
    "Confirming with e-ID isn't available for this form yet. Nothing more is needed — your form is finished.",
  'public.eid.offer':
    'You can confirm who you are with e-ID. It is optional: your form is already sent.',
  'public.eid.testNote':
    "Test mode: this uses a development provider and does not check anyone's identity.",
  'public.eid.start': 'Confirm with e-ID',
  'public.eid.waiting': 'Waiting for the e-ID app…',
  'public.eid.confirmed': 'Confirmed as {name}. Your document now says so.',
  'public.eid.confirmedTest':
    'Test confirmation recorded. It is not an identity check, and your document says so.',
  'public.eid.already': 'This form is already confirmed with e-ID.',
  'public.eid.failed': "The e-ID check didn't complete. Your form is still finished.",
  'public.eid.retry': 'Try again',

  'validation.required': 'This field is required.',
  'validation.tooShort': 'At least {min} characters.',
  'validation.tooLong': 'At most {max} characters.',
  'validation.pattern': 'Wrong format.',
  'validation.phone': 'Enter a phone number.',
  'validation.email': 'Enter an email address.',
  'validation.number': 'Enter a number.',
  'validation.min': 'At least {min}.',
  'validation.max': 'At most {max}.',
  'validation.decimals': 'At most {decimals} decimal places.',
  'validation.precision':
    'That number is too long to store exactly. Use a text field for long numbers.',
  'validation.date': 'Enter a date.',
  'validation.dateMin': 'No earlier than {min}.',
  'validation.dateMax': 'No later than {max}.',
  'validation.option': 'Choose a valid option.',
  'validation.minSelected': 'Choose at least {min}.',
  'validation.maxSelected': 'Choose at most {max}.',
  'validation.yesNo': 'Choose yes or no.',
  'validation.rating': 'Choose 1 to {max}.',
  'validation.time': 'Enter a time.',
  'validation.timeMin': 'No earlier than {min}.',
  'validation.timeMax': 'No later than {max}.',

  /**
   * Repeating blocks, and the guests they can admit.
   *
   * `docs/adr/0003-repeating-groups.md` has the design. The wording deliberately says "block"
   * rather than "group": the palette already groups field types, and an author reading "group"
   * would reasonably think of those.
   */
  'group.add': 'Add another',
  'group.remove': 'Remove',
  'group.removeEntry': 'Remove {entry}',
  'group.entryHeading': '{label} {number}',
  'group.empty': 'Nothing added yet.',
  'group.full': 'That is as many as this form allows ({max}).',
  'fieldType.repeating_group': 'Repeating block',
  'palette.repeating': 'Repeating',
  'field.fields': 'Questions in this block',
  'field.fieldsHint': 'Select one to edit it. Every question here is asked once per entry.',
  'field.minEntries': 'Fewest entries',
  'field.maxEntries': 'Most entries',
  'field.maxEntriesHint':
    'Required. The export has one set of columns per entry, so the form has to say how many.',
  'field.addLabel': 'Button text',
  'field.entryLabel': 'What one entry is called',
  'field.entryLabelHint': 'Drawn as a heading above each one — “Guest 1”, “Guest 2”.',
  'field.admits': 'Each entry gets its own admission card',
  'field.admitsHint':
    'For guests at an event. They are checked in separately from the person who registered, and only one block per form can do this.',
  'field.admitNameKey': 'Which question is their name',
  'field.admitNameKeyHint': 'Printed on the card and shown on the door screen.',
  'problem.group-min-above-max':
    'The block “{key}” asks for at least {min} entries but allows at most {max}.',
  'problem.group-duplicate-child-key':
    'The field key “{key}” is used more than once inside “{group}”.',
  'problem.group-admit-name-unknown':
    '“{group}” names “{key}” as the attendee, but has no such question.',
  'problem.multiple-admitting-groups':
    'Only one block on a form can give its entries their own admission card.',
  'validation.group': 'This answer is not in the expected form.',
  'validation.groupMin': 'Add at least {min}.',
  'validation.groupMax': 'At most {max}.',
  'checkin.outcome.no-such-guest': 'Guest not on this registration',
  'submissions.column.entry': '{group} {number} — {child}',
  'checkin.guestOf': 'Guest of {name}',

  /**
   * Bringing a form in from somewhere else.
   *
   * The reasons are listed one by one rather than counted: "3 questions were skipped" sends an
   * author hunting, and reporting *why* is the whole point of an importer that refuses to guess.
   */
  'import.open': 'Import',
  'import.heading': 'Import a SurveyJS survey',
  'import.explain':
    'Paste the survey’s JSON. Questions that have no equivalent here are listed rather than guessed at.',
  'import.paste': 'Survey JSON',
  'import.notASurvey': 'That is not a survey this can read.',
  'import.willImport':
    'plural:one {count} question will be imported | other {count} questions will be imported',
  'import.willSkip':
    'plural:one {count} question has no equivalent here | other {count} questions have no equivalent here',
  'import.replaces': 'Importing replaces everything currently in this form.',
  'import.confirm': 'Replace this form',
  'import.cancel': 'Cancel',
  'import.reason.no-equivalent': 'no equivalent here',
  'import.reason.unknown-type': 'an unknown question type',
  'import.reason.unreadable': 'nothing readable to import',
  'import.reason.needs-asset': 'its picture lives somewhere else on the web',
  'import.reason.nested-group': 'a repeating block inside another one',
  'import.reason.needs-limit': 'it never says how many entries it allows',
  /**
   * A form from the paper somebody already has — `screens/builder/paper/`.
   *
   * Never "scan" and never "sign". The first promises edge detection and OCR this does not do;
   * the second claims a legal act (`SPEC-forms.md` §8). It reads what the file declares, and the
   * author draws the rest.
   */
  /** The filled-in sheet, per row — a form made from paper only. */
  'submissions.document': 'Document',
  'submissions.paper': 'Paper',
  'submissions.noPaper': 'Filled in before this form had paper',

  'paper.open': 'From paper',
  'paper.heading': 'Make a form from paper',
  'paper.explain':
    'Upload the PDF you already have, or photograph each page. Fields the PDF declares are read as they are; on a photograph you draw the boxes yourself.',
  'paper.choose': 'PDF or photographs',
  'paper.reading': 'Reading…',
  'paper.storing': 'Saving…',
  'paper.pages': 'plural:one {count} page | other {count} pages',
  'paper.fieldsFound': 'plural:one {count} field found | other {count} fields found',
  'paper.noFields': 'No fields declared — draw them on the page after importing.',
  'paper.skipped': 'plural:one {count} field was skipped | other {count} fields were skipped',
  'paper.tooManyPages': 'That is {count} pages; the most this can take is {max}.',
  'paper.notReadable': 'That file could not be read.',
  'paper.tooCostly':
    'That PDF is too complex to open here. Save it again from the program that made it, or print it to a new PDF, and try that.',
  'paper.reason.no-answer': 'a button, which collects nothing',
  'paper.reason.not-a-question': 'filled in by the document, not the reader',
  'paper.reason.unreadable': 'a choice with no readable options',
  'paper.reason.unknown-type': 'an unknown field type',
  'paper.view': 'View',
  'paper.view.paper': 'Paper',
  'paper.view.list': 'List',
  'paper.draw':
    'Drag on a page to add a question there. Arrow keys move the selected box; Shift and arrows resize it.',
  'paper.pickType': 'What kind of answer?',
  'paper.page': 'Page {n}',
  'paper.crop':
    'Drag the corners onto the page’s corners. Arrow keys move the selected corner; Shift moves it further.',
  'paper.cropReset': 'Use the whole picture',
  'paper.cropNotAPage': 'Those corners do not make a page.',
  'paper.corner.0': 'Top-left corner',
  'paper.corner.1': 'Top-right corner',
  'paper.corner.2': 'Bottom-right corner',
  'paper.corner.3': 'Bottom-left corner',
  // Signing (P1c-3): sending a PDF to Loppa Sign.
  'nav.signing': 'Signing',
  'signing.heading': 'Signing',
  'signing.new': 'Send a PDF for signing',
  'signing.notConfigured': 'Signing is not set up',
  'signing.notConfiguredHint': 'Connect a Loppa Sign server to send documents for signing.',
  'signing.empty': 'Nothing sent for signing yet',
  'signing.emptyHint': 'Send a PDF, and each signer gets a link of their own.',
  'signing.file': 'PDF file',
  'signing.documentName': 'Document name',
  'signing.parties': 'Signers',
  'signing.partyName': 'Name',
  'signing.partyEmail': 'Email (optional)',
  'signing.partyLocale': 'Language',
  'signing.addParty': 'Add signer',
  'signing.removeParty': 'Remove',
  'signing.routing': 'Order',
  'signing.routing.sequential': 'One after another',
  'signing.routing.parallel': 'All at once',
  'signing.declarationKey': 'Declaration',
  'signing.testMode': 'Test mode',
  'signing.testModeHint': 'Nothing signed in test mode is marked as real.',
  'signing.send': 'Send for signing',
  'signing.sending': 'Sending…',
  'signing.failed': 'Could not send. Check the file and try again.',
  'signing.document': 'Document',
  'signing.status': 'Status',
  'signing.status.draft': 'Draft',
  'signing.status.sent': 'Waiting for signatures',
  'signing.status.completed': 'Signed by everyone',
  'signing.status.declined': 'Declined',
  'signing.status.expired': 'Expired',
  'signing.status.cancelled': 'Cancelled',
  'signing.party.waiting': 'Waiting',
  'signing.party.invited': 'Invited',
  'signing.party.viewed': 'Opened',
  'signing.party.signed': 'Signed',
  'signing.party.declined': 'Declined',
  'signing.openLink': 'Open signing page',
  'signing.refresh': 'Refresh',
  'signing.downloadSigned': 'Download signed PDF',
  'signing.declarationHint':
    'The words each signer approves. Your organisation writes them under Declarations.',
  'signing.declarations': 'Declarations',
  'signing.declarationsHint':
    'What your signers approve when they sign, in each language. Loppa never writes these words: an admin in your organisation types them. Saving makes a new version; documents already sent keep the words they were sent with.',
  'signing.declarationVersion': 'Version {version}',
  'signing.declarationTestOnly': 'Test only',
  'signing.declarationWriteOwn': 'Write your own',
  'signing.declarationNewVersion': 'New version',
  'signing.declarationNew': 'New declaration',
  'signing.declarationAdminOnly': 'Only an admin can write a declaration.',
  'signing.declarationEditor': 'Write a declaration',
  'signing.declarationKeyLabel': 'Short name',
  'signing.declarationKeyHint':
    'Lower-case letters, digits, dots, dashes and underscores, e.g. lease-2026.',
  'signing.declarationOwnWords':
    'Signers see exactly what you type here, word for word. Leave a language empty if you do not use it.',
  'signing.declarationConfirm':
    'Signers will approve exactly these words. Press again to save them as a new version.',
  'signing.declarationFailed': 'Could not save the declaration. Try again.',
  'signing.declarationSave': 'Save',
  'signing.declarationSaveConfirm': 'Yes, save this version',
  'signing.environment': 'Mode',
  'signing.realMode': 'Real',
  'signing.declarationMissingLanguage':
    'The declaration has no words in {languages}. Add them under Declarations, or choose another language for that signer.',
  'signing.realConfirm':
    "I have checked the declaration in every signer's language, and this is a real signing, not a test.",
  'signing.realNeedsOwnDeclaration':
    'Real signing needs a declaration your organisation wrote itself. Write one under Declarations.',
  'signing.fileHint': 'A PDF, or pages from a scanner as JPEG or PNG images.',
  'signing.inviteByEmail': 'Email each signer their link when it is their turn',
  'signing.inviteConfirm':
    'Send an email with a signing link to {count} signers as their turn comes? Signers without an address get nothing; copy their links instead.',
  'signing.inviteConfirmYes': 'Send and email',
  'signing.remind': 'Send reminder',
  'signing.remindConfirm': 'Email {name} ({email}) their signing link again?',
  'signing.emailed': 'Emailed',
  'signing.cancel': 'Cancel',
  'submissions.sendForSigning': 'Send for signing',
  'signing.fromSubmission': 'Filled-in paper form {reference}',
  // Scanning with a camera (phone or PC).
  'phone.use': 'Use a phone',
  'phone.heading': 'Scan with your phone',
  'phone.opening': 'Getting a code for your phone…',
  'phone.noNetwork':
    'This computer is not on a network a phone can reach. Connect it to Wi-Fi and try again.',
  'phone.failed': 'Could not start scanning with a phone. Try again.',
  'phone.expired': 'The code has expired.',
  'phone.hint':
    "Point your phone's camera at the code and open the link. The phone must be on the same Wi-Fi as this computer.",
  'phone.qrAlt': 'QR code with the link for your phone',
  'phone.received': 'Pages received: {count}',
  'phone.useReceived': 'Use the pages ({count})',
  'phone.newCode': 'New code',
  'phone.back': "Use this computer's camera",
  'phone.page.heading': 'Scan for your computer',
  'phone.page.gone': 'This link has expired or was closed. Show a new code on the computer.',
  'phone.page.sending': 'Sending page {number} of {count}…',
  'phone.page.intro':
    'Take a photo of each page. The pages go straight to the computer that showed the code.',
  'phone.page.full': 'The computer already has as many pages as one scan can take.',
  'phone.page.failed': 'A page could not be sent. Try again.',
  'phone.page.sent': 'Sent. The computer has {count} pages. You can put the phone away.',
  'phone.page.more': 'Scan more pages',
  'camera.open': 'Use camera',
  'camera.scanDocument': 'Scan with camera',
  'camera.preview': 'Camera preview',
  'camera.starting': 'Starting the camera…',
  'camera.capture': 'Take photo',
  'camera.capturePage': 'Take page',
  'camera.choose': 'Camera',
  'camera.default': 'Default camera',
  'camera.numbered': 'Camera {number}',
  'camera.pages': 'Scanned pages ({count})',
  'camera.page': 'Page {number}',
  'camera.remove': 'Remove',
  'camera.done': 'Done ({count})',
  'camera.cancel': 'Cancel',
  'camera.systemCamera': 'Open the camera',
  'camera.unavailable.insecure':
    "The live camera needs a secure connection here. Use the device's own camera instead.",
  'camera.unavailable.denied':
    "The camera was not allowed. Allow it in the browser or system settings, or use the device's own camera.",
  'camera.unavailable.none': "No camera was found. Connect one, or use the device's own camera.",
  'camera.unavailable.failed':
    "The camera could not be started. Try again, or use the device's own camera.",
  'signing.source.file': 'PDF file',
  'signing.source.camera': 'Camera',
  'nav.outgoing': 'To send',
  'outgoing.heading': 'To send',
  'outgoing.intro':
    'Nothing is sent automatically. Each message waits here until you open it in your email program and press Send there.',
  'outgoing.empty': 'Nothing is waiting to be sent',
  'outgoing.emptyHint':
    'Confirmations, admission cards and signing invitations appear here when there is something to send.',
  'outgoing.to': 'To',
  'outgoing.waitingOne': 'Waiting',
  'outgoing.opened': 'Opened {when}',
  'outgoing.showText': 'Message',
  'outgoing.attachNote':
    'An email link cannot attach files. Save the attachment and attach it to the message.',
  'outgoing.openIn': 'Open in {program}',
  'outgoing.openEmailApp': 'Open in email app',
  'outgoing.openFailed': '{program} could not open the message. Use the email app instead.',
  'outgoing.remove': 'Remove',
  'outgoing.removeConfirm': 'Remove the message to {to}? It will not be sent.',
  'links.localOnly': 'Links open only on this computer. Nobody else can open them.',
  'links.copiedLocal': 'Copied (only on this computer)',
  // The guided builder's conversation: packages/shared/src/builder/graph/nodes.ts. Its rules
  // (at most nine words a question, no jargon) are checked by guided-graph.test.ts.
  'guided.common.yes': 'Yes',
  'guided.skip.decided': 'Already read from your document',
  'guided.skip.noButtons': 'You chose no buttons',
  'guided.skip.brandDecided': 'Already decided',
  'guided.skip.nothingToGuess': 'Nothing to guess yet',
  'guided.preview.ask': "Here's how it looks.",
  'guided.guess.ask': 'This looks like {template}. Right?',
  'guided.guess.help': 'Loppa guessed from your answers. You can say no.',
  'guided.guess.right': 'Right',
  'guided.guess.sortOf': 'Sort of',
  'guided.guess.no': 'No',
  'guided.flow.start.ask': 'What is this form for?',
  'guided.flow.start.help': 'Your answer helps Loppa suggest the right questions.',
  'guided.flow.start.signup': 'Signing people up',
  'guided.flow.start.signupDetail': 'Events, courses and meetings.',
  'guided.flow.start.collect': 'Collecting information',
  'guided.flow.start.collectDetail': 'Members, orders and applications.',
  'guided.flow.start.feedback': 'Hearing what people think',
  'guided.flow.start.feedbackDetail': 'Feedback, surveys and suggestions.',
  'guided.flow.start.other': 'Something else',
  'guided.flow.start.otherDetail': 'Start with one question and build from there.',
  'guided.flow.more.ask': 'Add another question?',
  'guided.flow.more.help': 'You can always add more later.',
  'guided.flow.more.yes': 'Yes, ask something else',
  'guided.flow.more.no': "No, that's everything",
  'guided.brand.start.ask': 'Should this look like your organisation?',
  'guided.brand.start.help': 'Uses the logo and colours from your brand kit.',
  'guided.brand.start.yes': 'Yes, use our colours and logo',
  'guided.brand.start.later': 'Decide later',
  'guided.brand.quick.ask': 'Which colours should your form use?',
  'guided.brand.quick.help': 'You can change them any time.',
  'guided.brand.quick.minimal': 'Minimal',
  'guided.brand.quick.garden': 'Garden',
  'guided.brand.quick.bold': 'Bold',
  'guided.brand.quick.midnight': 'Midnight',
  'guided.brand.logoSlot.ask': 'Where should your logo go?',
  'guided.brand.logoSlot.help': 'Pick a place. It looks right on phones too.',
  'guided.brand.logoSlot.headerLeft': 'Top left, beside the title',
  'guided.brand.logoSlot.mastheadCentred': 'Centred above the title',
  'guided.brand.logoSlot.cornerWatermark': 'Small, in the corner',
  'guided.brand.logoSlot.footerStrip': 'In a strip at the bottom',
  'guided.brand.logoSlot.sidebarRail': 'In a column beside the form',
  'guided.brand.logoSlot.cardTop': 'At the top of the card',
  'guided.brand.preview.help': 'This is your form as people will see it.',
  'guided.text.label.ask': 'What do you want to ask?',
  'guided.text.label.help': 'Write it the way you would say it.',
  'guided.text.label.exampleName': 'Your name',
  'guided.text.label.exampleEmail': 'Your email address',
  'guided.text.label.exampleDay': 'Which day suits you?',
  'guided.text.required.ask': 'Must everyone answer this?',
  'guided.text.required.help': "People can't send the form until they answer.",
  'guided.text.required.yes': "Yes, it's needed",
  'guided.text.required.no': "No, it's optional",
  'guided.choice.buttons.ask': 'Do you want buttons?',
  'guided.choice.buttons.help': 'Buttons let people pick instead of typing.',
  'guided.choice.buttons.no': 'No, people type an answer',
  'guided.choice.answers.ask': 'One answer or several?',
  'guided.choice.answers.help': 'Can people pick more than one?',
  'guided.choice.answers.one': 'One answer only',
  'guided.choice.answers.many': 'Several answers allowed',
  'guided.choice.count.ask': 'How many options?',
  'guided.choice.count.help': 'You can add or remove options later.',
  'guided.choice.shape.ask': 'What shape?',
  'guided.choice.shape.help': 'The shape of each button.',
  'guided.choice.shape.pill': 'Pill',
  'guided.choice.shape.rounded': 'Rounded',
  'guided.choice.shape.square': 'Square',
  'guided.choice.shape.tile': 'Cards, with room for a picture',
  'guided.choice.placement.ask': 'Where should they sit?',
  'guided.choice.placement.help': 'How the buttons are laid out.',
  'guided.choice.placement.underFull': 'Under the question, full width',
  'guided.choice.placement.row': 'Side by side',
  'guided.choice.preview.help': 'Click it to change anything by hand.',
  'guided.menu.top.ask': 'What do you want to change?',
  'guided.menu.top.help': 'Jump to any part of your form.',
  'guided.end.ask': 'Your form is ready.',
  'guided.end.help': 'Publish it now, or keep adding questions.',
} as const;

/**
 * Every message the app can ask for.
 *
 * Derived from the English catalogue rather than declared separately, so the reference and
 * the type cannot drift. Every other locale is typed `Record<MessageKey, string>`, which makes
 * a missing translation a compile error instead of a blank label somebody finds in production.
 */
export type MessageKey = keyof typeof enGB;
