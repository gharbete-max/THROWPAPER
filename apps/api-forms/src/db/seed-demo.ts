...
/**
 * The demo dataset, written into whichever database it is given.
 *
 * A function rather than a top-level script so the desktop edition can offer the same demo on its
 * embedded database (ADR 0016) that `pnpm db:seed` writes to Postgres. Idempotent: a second run
 * inserts nothing new.
 */
export async function seedDemo(db: Db): Promise<{ organisationId: string; formSlug: string }> {
  /**
  ...
  return { organisationId: organisation.id, formSlug: DEMO_FORM_SLUG };
}
