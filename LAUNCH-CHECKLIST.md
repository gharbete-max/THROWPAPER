...
| Two filled buttons on the public form's last page at 375: "Add a guest" (343×44) directly above the submit. **Root cause found 2026-09-22:** `button--secondary` is asked for in `RepeatingGroup.tsx:177` and defined in no stylesheet, so it renders as the primary. Define the tier or use `button--quiet` — one rule closes the row, and any other screen reaching for a secondary gets a second primary today | `components/RepeatingGroup.tsx:177`, `styles.css` |
...
| **P2, measured 2026-09-22:** the verdict panel's fixed height holds at 375 (224px in all four states) but not at 1280, where "already" is 194.78px against 176px — the panel jumps between the admit and the repeat scan                                                                                                                                                                                          | `styles.css` `.verdict`                           |
| **P2, measured 2026-09-22:** `/responses` at 375 clips by the same mechanism as the door's row — 13 `text-overflow` hits on `inbox__who`, `inbox__form` and `inbox__reference`; the reference is the field that matches a human to a card                                                                                                                                                                         | `styles.css` `.inbox__*`                          |
...
| What                                                                                                                                                                                                                                | Where                                      |
| ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------ |
| **P1** A filled button has no hover or pressed state anywhere in the product: the ink swap at `styles.css:149` is out-specified by the `--tp-button-*` paint at `:3517` (equal specificity, later). Only the shadow moves 22% → 28% | `styles.css` `.button:hover`, `:3517`      |
| **P1** The quiet button's hover edge is `--tp-colour-primary` — gold on paper at 2.14:1, weaker than its resting pewter at 3.75:1. A brand colour painting a boundary unchecked                                                     | `styles.css:182` `.button--quiet:hover`    |
