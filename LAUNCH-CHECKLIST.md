...
| Email handoff on Windows (Outlook COM `Display()`) and macOS (Mail/Outlook AppleScript) — including the desktop's default **To send → Open in Outlook / Apple Mail**, which now also sets the recipient — verified by tests of the scripts, not by a person on those machines                              | no Windows/Mac here                                                                                              | —        |
| Sending through Apple Mail / Outlook for Mac deletes the attachment folder as soon as the script returns; `draft.ts` records that Mail reads an attachment at send time, so a confirmation may leave without its PDF. Test on a Mac first; if it does, keep the folder until the daily sweep, as drafts do | no Mac here — found by reading `outlook.ts` beside `draft.ts` (`docs/PROGRESS.md` § Release-readiness audit)     | medium   |
| Five downloads revoke their blob URL straight after `click()` (a response's file, invoices, the CSV, the attendance list, a signed PDF); `saveBlob` waits a minute because a revoked URL can fail a download in some browsers. Chromium (and so the desktop) is fine; check Firefox and Safari             | only Chromium here                                                                                               | low      |
...
the last two. **Two rows remain, both waiting on you.** The third, the door's recent arrivals
vanishing on reload, closed in #128: the server keeps each operator's last five.
...
| **Desktop: code-signing certificate** (or Microsoft's signing service) and whose name is on it — SmartScreen warns until then. Wired: set the `WIN_CSC_LINK` / `WIN_CSC_KEY_PASSWORD` secrets                                                               | D1 release    | you           |
| **Desktop: Apple Developer ID** to notarise the Mac app — Gatekeeper asks on first open until then. Wired: set `MAC_CSC_LINK` / `MAC_CSC_KEY_PASSWORD` and `APPLE_API_KEY` / `APPLE_API_KEY_ID` / `APPLE_API_ISSUER` (`docs/PROGRESS.md` § Desktop signing) | D1 release    | you           |
