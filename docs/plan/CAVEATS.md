...
| 52 | `operative-wording-never-generated` | an example chip or a seeded template writes a consent or safety sentence | chips are labels only; consent text comes from the organisation, or publishing is blocked (ADR 0012) | planned: `packages/shared/src/builder/graph/graph.test.ts` (G13, S1) |
